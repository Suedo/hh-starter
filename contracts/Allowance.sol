...
contract Allowance is ReentrancyGuard, Ownable {
  ...
  function giveAllowanceMoney(address user, uint256 amount)
    external
    onlyOwner
    nonReentrant
    returns (uint256)
  {
    ...
    require(
      allowance[user] >= balanceOf[user] + amount,
      "Cannot give more than allowance limit"
    );
    balanceOf[owner()] -= amount;
    balanceOf[user] += amount;
    emit AllowanceChange(
      user,
      allowance[user],
      ChangeType.Increase,
      balanceOf[user]
